...
ggplot(data=data,aes(RelDate,Price)) +
  geom_smooth(method="glm",method.args=list(family=gaussian(link="log")),color="orange") +
  geom_line() +
  geom_rangeframe() +
  theme_tufte() + 
...
```{r echo=F}
g1 = ggplot(data5,aes(zDate,Price,color=Boot)) +
  geom_line() +
  theme_bw() +
  theme(legend.position="None") +
  scale_y_continuous(limits=c(0,750)) +
  xlab('Time (days)')
g2= ggplot(data30,aes(zDate,Price,color=Boot)) +
  geom_line() +
  theme_bw() +
  theme(legend.position="None") +
  scale_y_continuous(limits=c(0,750)) +
...
```{r echo=F}
ggplot() +
  geom_point(data=fdata,aes(NumDCAEvents,EndPrice,color=NumDCAEvents),size=3) +
  geom_point(data=mfdata,aes(NumDCAEvents,mu),shape='-',size=40) +
...
```{r}
emote(c(1,2,3,4,5)) # continuously increasing, most happiness
emote(c(1,2,1,4,5)) # one dip, less happy
emote(c(1,0,-1,-2,5)) # many dips, very unhappy
emote(c(1,6,-5,3,5)) # large variability
```
...
```{r echo=F}
ggplot() +
  geom_point(data=fdata,aes(NumDCAEvents,Emotion,color=Boot),size=3,position=position_jitter(width=.3,height=.3)) +
  geom_point(data=mfdata,aes(NumDCAEvents,muE),shape='-',size=30) +
  xlab("Number of DCA Events") +
  ylab("Emotions (Positive is better)") +
  geom_rangeframe() +
  theme_bw() +
  theme(legend.position="none") +
...
(Apologies for the poor quality graphs, they come straight from R and don't look all that great...)